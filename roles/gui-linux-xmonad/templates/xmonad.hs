...
myStartupHook = do
    ...
    spawnOnce "compton"
    spawnOnce "launch_redshift --auto 6500:2500"
    spawnOnce "launch_volumecontrol"
    spawnOnce "albert"
...
main = do
    ...
    {% for idx in range(0, gui.monitors, 1) | list %}xmproc{{idx}} <- spawnPipe "xmobar -x {{ idx }} $HOME/.config/xmobar/xmobarrc-{{idx}}"
    {% endfor %}
    ...
    xmonad $ ewmh def
        ...
        , logHook = dynamicLogWithPP $ namedScratchpadFilterOutWorkspacePP $ xmobarPP
              ...
              { ppOutput = \x -> {% for idx in range(0,  gui.monitors, 1) | list %}{% if idx > 0 %}>>{% endif %} hPutStrLn xmproc{{idx}} x -- xmobar on monitor {{ idx + 1 }}
                                {% endfor %}
